...
// Initialize the library with the new 20x4 LCD address and pin configuration
...
#define BOY_POSITION_RUN_UPPER_2 12 // (pose 2)                        (pose 2)
...
bool drawBoy(byte position, char* terrainUpper, char* terrainLower, unsigned int score) {
  ...
  // Draw the scene
  lcd.setCursor(0, 1);  // Start drawing on the first row
  ...
  // Always draw the bottom row as solid terrain
  for (int i = 0; i < TERRAIN_WIDTH; ++i) {
   lcd.print('\5');
  }

  // Draw the boy and score on the top row
  lcd.setCursor(0, 0);
...
void loop() {
  ...
  if (!playing) {
    drawBoy(boyPos == BOY_POSITION_RUN_LOWER_2 ? BOY_POSITION_OFF : boyPos, terrainUpper, terrainLower, distance >> 3);
  lcd.setCursor(3,0);

      lcd.print("Push to  Start");
    if (boyPos == BOY_POSITION_RUN_LOWER_2) {
      lcd.setCursor(0, 3);
      for (int i = 0; i < TERRAIN_WIDTH; ++i) {
        lcd.print('\5');
      }
  ...
  if (drawBoy(boyPos, terrainUpper, terrainLower, distance >> 3)) {
    playing = false; // The boy collided with something. Game over.
  } else {
